...
namespace DataTableListView.Behaviors
{
    using System;
    ...
    public class TextBoxInputBehavior : Behavior<TextBox>
    {
        ...
        public TextBoxInputBehavior()
        {
            ...
            this.decimalSeparator = Convert.ToChar(this.cultureInfo.NumberFormat.NumberDecimalSeparator,CultureInfo.CurrentCulture);
            this.numberGroupSeparator = Convert.ToChar(this.cultureInfo.NumberFormat.NumberGroupSeparator, CultureInfo.CurrentCulture);
        ...
        private void AssociatedObjectLostFocus(object sender, RoutedEventArgs e)
        {
            if (this.InputMode == TextBoxInputMode.CurrencyInput)
            {
                ...
                {
                    decimal value = Convert.ToDecimal($"{this.AssociatedObject.Text}{this.decimalSeparator}00", CultureInfo.CurrentCulture);
                    this.AssociatedObject.Text = value.ToString($"N{this.DecimalPlace}", CultureInfo.CurrentCulture);
                }
                else
                {
                    decimal value = Convert.ToDecimal(this.AssociatedObject.Text.Replace(".",string.Empty), CultureInfo.CurrentCulture);
                    this.AssociatedObject.Text = value.ToString($"N{this.DecimalPlace}", CultureInfo.CurrentCulture);
                }
            }
            else if (this.InputMode == TextBoxInputMode.DecimalInput)
            {
                ...
                {
                    decimal value = Convert.ToDecimal("0", CultureInfo.CurrentCulture);
                    this.AssociatedObject.Text = value.ToString(CultureInfo.CurrentCulture);
                }
                else
                {
                    if (this.AssociatedObject.Text.Contains(this.decimalSeparator) == false)
                    {
                        decimal value = Convert.ToDecimal(this.AssociatedObject.Text, CultureInfo.CurrentCulture);
            ...
            else if (this.InputMode == TextBoxInputMode.DigitInput)
            {
                ...
                {
                    int value = Convert.ToInt32(this.AssociatedObject.Text, CultureInfo.CurrentCulture);
                    this.AssociatedObject.Text = value.ToString(CultureInfo.CurrentCulture);
                }
            }
        }
        ...
        private bool IsValidInput(string input)
        {
            ...
            switch (this.InputMode)
            {
                ...
                case TextBoxInputMode.DecimalInput:
                    ...
                    if (input.ToCharArray().Where(x => x == this.numberGroupSeparator).Any() == true)
                    {
                        decimal decimalOut;
        ...
        private static bool CheckIsLetterOrDigit(string wert)
        {
            return wert.ToCharArray().All(char.IsLetterOrDigit);
        }

        private static bool CheckIsLetter(string wert)
        {
            return wert.ToCharArray().All(char.IsLetter);
        }

        private static bool CheckIsdate(string value)
        {
            bool result = false;
            DateTime date;
