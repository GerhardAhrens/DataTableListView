...
namespace DataTableListView.Generator
{
    using System;
    ...
    public sealed class SqlBuilderContext : IDisposable
    {
        ...
        public (string, SQLiteParameter[]) GetInsert()
        {
            ...
            if (this.CurrentDataRow != null)
            {
                ...
                {
                    if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.KeyColumn.First().ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
                        columnNames.Add(column);
                        columnParamNames.Add($":{column.ColumnName}");
                    }
                    else if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.CreateByColumn.ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
                        columnNames.Add(column);
                        columnParamNames.Add($":{column.ColumnName}");
                    }
                    else if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.CreateOnColumn.ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
                        columnNames.Add(column);
                        columnParamNames.Add($":{column.ColumnName}");
                    }
                    else if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.ModifyByColumn.ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
                    }
                    else if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.ModifyOnColumn.ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
                    }
                    else
                    {
                ...
                sb.Append(CultureInfo.CurrentCulture, $"INSERT INTO {tableName}").Append(" (");
                ...
                {
                    if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.CreateByColumn.ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
        ...
        public (string, SQLiteParameter[]) GetUpdate()
        {
            ...
            if (this.CurrentDataRow != null)
            {
                ...
                sb.Append(CultureInfo.CurrentCulture, $"UPDATE {tableName} SET ");
                foreach (DataColumn column in columns)
                {
                    if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.KeyColumn.First().ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
                    ...
                    {
                        columnNames.Add(column);
                        sb.Append(CultureInfo.CurrentCulture, $"{column} = :{column}, ");
                    }
                    else if (column.ColumnName.ToLower(CultureInfo.CurrentCulture).Equals(this.ModifyOnColumn.ToLower(CultureInfo.CurrentCulture), StringComparison.Ordinal) == true)
                    {
                        columnNames.Add(column);
                ...
                sb.Append(CultureInfo.CurrentCulture, $" WHERE {this.KeyColumn.First()} = :{this.KeyColumn.First()}");
        ...
        public (string, SQLiteParameter[]) GetDelete()
        {
            ...
            if (this.CurrentDataRow != null)
            {
                ...
                sb.Append(CultureInfo.CurrentCulture,$"DELETE FROM {tableName}");
                sb.Append(CultureInfo.CurrentCulture,$" WHERE {this.KeyColumn.First()} = :{this.KeyColumn.First()}");
