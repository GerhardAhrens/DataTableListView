...
//
// <summary>SQLite Extension</summary>
//-----------------------------------------------------------------------
...
namespace System.Data.SQLite
{
    ...
    public static class SQLRecordSetExtension
    {
                ...
                using (SQLiteCommand cmd = connection.CreateCommand())
                {
                    ...
                    using (SQLiteDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.HasRows == true && dr.VisibleFieldCount > 0)
                        {
                            dt = new DataTable();
                            dt.Load(dr);
                            if (HasColumn(dt,"CreatedOn") == true && HasColumn(dt,"CreatedBy") == true && HasColumn(dt,"ModifiedOn") == true && HasColumn(dt, "ModifiedBy") == true)
                            {
                                if (HasColumn(dt, "Timestamp") == false)
                                {
                                    ...
                                    foreach (DataRow row in dt.Rows)
                                    {
                                        ...
                                        string timestamp = TimeStamp.MaxEntry(createdOn, row["CreatedBy"].ToString(), modifiedOn, row["ModifiedBy"].ToString());
                                        row.BeginEdit();
        ...
        private static string ExtractTablename(string sql)
        {
            ...
            {
                return $"TAB_{DateTime.Now.ToString("yyyyMMdd", CultureInfo.CurrentCulture)}";
            }
        }
        ...
        public static bool HasColumn(DataTable @this, string columnName)
        {
            bool result = false;
    ...
    {
        public static string MaxEntry(DateTime createdOn, string createdBy, DateTime modifiedOn, string modifiedBy)
        {
